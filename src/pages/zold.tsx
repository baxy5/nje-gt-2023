...
export const getStaticProps: GetStaticProps = async () => {
  const zold = await prisma.csapat.findMany({
    where: {
      name: 'Zöld csapat',
    },
  });
...
const zold = ({ zold }) => {
  ...
  const id = zold[0].id;
  const prevDesc = zold[0].description;
  ...
  return (
    <div>
      <div className="border-2 border-black rounded-md p-4" style={{ backgroundColor: teamColorCheck(zold[0].name) }}>
        <h1 className="text-[2rem] text-center text-[#FFFFFF]">
          {zold[0].name}:<span className="pl-2">{zold[0].points}</span>
        </h1>
      </div>
      <div>
        ...
        <form action="" className="py-4 px-4">
          ...
          <div className="flex items-center justify-center gap-4 pt-6">
            <button
              ...
              style={{ backgroundColor: teamColorCheck(zold[0].name) }}
