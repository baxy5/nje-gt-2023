...
export const getStaticProps: GetStaticProps = async () => {
  const rozsaszin = await prisma.csapat.findMany({
    where: {
      name: 'Rózsaszín csapat',
    },
  });
  return {
    props: { rozsaszin },
...
const rozsaszin = ({ rozsaszin }) => {
  ...
  const id = rozsaszin[0].id;
  const prevDesc = rozsaszin[0].description;
